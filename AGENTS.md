...
- **财务到款凭证匹配已按 DOC-001 落地**：Phase 1（凭证匹配向导 + 发票组合算法 + `FinanceReceiptAllocation` 分摊表 + 聚合口径重写 + 冲红/删除/编辑约束）、Phase 2（回款流水“凭证匹配”筛选与 CSV 导出）、Phase 3（银行流水批量导入 + 自动解析付款单位 + 匹配复核队列）均已完成并通过 smoke/build。
