...
import { FolderTree, Receipt, UserRound, Filter, X, FileText, Trash2, Merge, Tags } from "lucide-react";
    ...
    <div className="p-4 md:p-6 2xl:px-8 space-y-4 w-full">
      ...
      {showBatchBar && (
        <div className="hidden md:flex fixed bottom-4 right-4 z-40 items-center gap-3 bg-background border shadow-lg rounded-lg px-4 py-3">
          ...
          <Button size="sm" variant="outline" className="h-8 text-xs" onClick={() => setBatchCategoryOpen(true)}>
            <Tags className="h-3 w-3 mr-1" />批量改类型
          </Button>
      ...
      {showBatchBar && (
        ...
        >
          ...
          <Button size="sm" variant="outline" className="h-7 text-xs flex-1" onClick={() => setBatchCategoryOpen(true)}>
